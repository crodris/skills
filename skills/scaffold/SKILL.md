...
4. Gather the requirements before drafting.
   ...
   - When the user names the current conversation as the source, synthesize the requirements from it instead of interviewing them; step 6 still stops on two incompatible readings.
...
8. Create the scaffold, once approved in ask mode or immediately in auto mode.
   ...
   - Creating blockers first gives each later sub-issue real refs to name, so end each sub-issue description with a `Blocked by:` line listing its blockers' refs, or `none`.
