...
11. Once `claimNext` returns none remaining, finish the issue.
    ...
    - Call `openReview` with the branch, the resolved base, a title, and a body.
      Title it the way the repository titles its merged reviews, naming the outcome for the user: `perf(server): cut websocket frame size by 70%+ with gzipping` names the outcome, where `perf(server): negotiate permessage-deflate on the websocket` names only the mechanism.
