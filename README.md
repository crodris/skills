...
- **Two reviewers, not one twice** - the pre-merge review is a Code Reviewer subagent reading this run's intent, and the pull-request bot is the final bar that still has to settle green; ship never shells out to a review CLI, because the vendors that ship one also run the bot and the CLI would spend that quota on a judgment the bot reaches anyway; a bot that reviews only the first push is asked for one more review only after a fix for a critical or major finding it raised, and the subagent confirms every other fix
